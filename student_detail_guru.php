...
$page_title = 'Detail Siswa - SIAKAD Guru';
...
// Ambil data guru
$user_data = supabase_admin_request('GET', 'users', null, ['id' => 'eq.' . $teacher_id]);
...
$jadwal_raw = supabase_admin_request('GET', 'schedules', null, ['teacher_id' => 'eq.' . $teacher_id]);
$jadwal = safeArray($jadwal_raw);

if (empty($jadwal)) {
    $error_message = "Anda belum memiliki jadwal mengajar. Tidak ada siswa yang dapat dilihat.";
    $all_students = [];
    $class_map = [];
} else {
    ...
    // Ambil semua siswa yang memiliki kelas yang diajar
    $all_students_raw = supabase_admin_request('GET', 'users', null, ['role' => 'eq.student']);
    $all_students = is_array($all_students_raw) ? $all_students_raw : [];
    
    // Filter siswa berdasarkan kelas yang diajar
    $all_students = array_filter($all_students, function($s) use ($teacher_class_ids) {
        return in_array($s['kelas_pagi_id'] ?? 0, $teacher_class_ids) || 
               in_array($s['kelas_diniyyah_id'] ?? 0, $teacher_class_ids);
    });
    ...
    <div class="main-content-container flex-1 transition-all duration-300 ease-in-out">
        ...
        <main class="p-4 md:p-6 dark:bg-gray-900 transition-colors">
            <?php if (isset($error_message)): ?>
                <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4 mb-6 text-yellow-700 dark:text-yellow-400">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                        ...
                        <?php if (!empty($teacher_class_ids)): ?>
                            <div class="mb-3">
                                <select name="class_id" form="filterForm" class="w-full border rounded-lg px-3 py-1.5 text-sm dark:bg-gray-700 dark:border-gray-600">
                                    ...
                                    <?php foreach ($teacher_class_ids as $cid): ?>
                                        <?php if (isset($class_map[$cid])): ?>
                                            <option value="<?= $cid ?>" <?= $class_filter == $cid ? 'selected' : '' ?>><?= htmlspecialchars($class_map[$cid]) ?></option>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                                    ...
                                    <a href="?student_id=<?= $s['id'] ?>&search=<?= urlencode($search) ?>&class_id=<?= $class_filter ?>&page=<?= $page ?>" 
                                       class="student-list-item <?= $is_active ? 'active' : '' ?> block px-3 py-2 rounded-lg mb-1 text-gray-800 dark:text-white">
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            ...
                                            <?php if (!empty($s['kelas_pagi_id']) && isset($class_map[$s['kelas_pagi_id']])): ?>
                                                • <?= htmlspecialchars($class_map[$s['kelas_pagi_id']]) ?>
                                            <?php endif; ?>
                                        </div>
                                    </a>
                                <?php endforeach; ?>
                ...
                <div class="lg:col-span-3">
                    <?php if ($selected_student && $selected_student_id): ?>
                        <?php 
                            $kelas_pagi = !empty($selected_student['kelas_pagi_id']) && isset($class_map[$selected_student['kelas_pagi_id']]) ? $class_map[$selected_student['kelas_pagi_id']] : '-';
                            $kelas_diniyyah = !empty($selected_student['kelas_diniyyah_id']) && isset($class_map[$selected_student['kelas_diniyyah_id']]) ? $class_map[$selected_student['kelas_diniyyah_id']] : '-';
                        ?>
                        <!-- Profil -->
                    ...
                    <?php elseif ($selected_student_id): ?>
                        <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4 text-yellow-700 dark:text-yellow-400">
                            <i class="fas fa-exclamation-triangle mr-2"></i> Siswa tidak ditemukan dalam daftar siswa yang Anda ajar.
                        </div>
                    <?php else: ?>
...
document.getElementById('openSidebarUserBtn')?.addEventListener('click', function() {
    ...
    document.getElementById('sidebarUser').classList.add('translate-x-0');
